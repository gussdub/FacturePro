...
class TestCheckAndBillScan:
    """Task 8 (feature #11) — the scan quota lives on the organization now,
    not on the user. _check_and_bill_scan takes an organization_id."""

    def _setup_org(self, scan_count=0, reset_at=""):
        oid = f"test-quota-org-{uuid.uuid4().hex[:8]}"
    ...
    def _cleanup(self, oid):
        server_db.organizations.delete_one({"id": oid})

    def test_first_scan_ever_increments_to_1(self):
        oid = self._setup_org(scan_count=0, reset_at="")
        try:
            count = _check_and_bill_scan(oid)
            assert count == 1
            org = server_db.organizations.find_one({"id": oid})
            assert org["scan_count_this_month"] == 1
            assert org["scan_quota_reset_at"]
        finally:
            self._cleanup(oid)

    def test_increment_within_month(self):
        now_iso = datetime.now(timezone.utc).isoformat()
        oid = self._setup_org(scan_count=5, reset_at=now_iso)
        try:
            count = _check_and_bill_scan(oid)
            assert count == 6
        finally:
    ...
    def test_reset_when_month_changed(self):
        ...
        oid = self._setup_org(scan_count=199, reset_at=old_iso)
        try:
            count = _check_and_bill_scan(oid)
            assert count == 1
            org = server_db.organizations.find_one({"id": oid})
